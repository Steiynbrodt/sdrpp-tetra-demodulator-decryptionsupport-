...
Do not reuse a build directory previously configured from the repository root;
CMake caches its source directory and old project control flow in that build
tree. The normal SDR++ plugin continues to use the unchanged root
`CMakeLists.txt` and the repository's regular build instructions above.
